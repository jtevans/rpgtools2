    ...
    return (
      <div>
        {names[column].map((name, idx) => (
          <div>
            {name}<ReplayIcon sx={{ paddingLeft: "5px", fontSize: "9pt" }} onClick={() => replaceName({ column, idx })} />
          </div>
        ))}
      </div>
    )
      ...
      >
        <Box sx={style} ref={gridRef}>
          ...
          <Typography sx={{ textAlign: "center" }} variant="h5" component="h2">
            <ContentCopyIcon sx={{ paddingRight: "5px", fontSize: "12pt" }} onClick={handleCopy} />Dwarf Names<ReplayIcon sx={{ paddingLeft: "5px", fontSize: "10pt" }} onClick={() => getNames()} />
          </Typography>
